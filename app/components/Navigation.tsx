  ...
  return (
    ...
    >
      ...
      {mobileMenuOpen && typeof document !== 'undefined' && (
        <div 
          className="fixed inset-0 z-[100] md:hidden bg-[var(--background)] flex flex-col"
          style={{ height: '100dvh', width: '100vw' }}
        >
          <div className="container mx-auto px-6 py-5 flex flex-col h-full">
            ...
            <div className="flex-1 flex flex-col justify-center min-h-0">
              <span className="text-[11px] font-black tracking-[0.25em] opacity-40 uppercase block mb-3 ml-1">Quick Navigation</span>
              <div className="grid grid-cols-1 gap-2">
                {links.map((link, index) => (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className={`flex items-center justify-center py-4 px-6 rounded-2xl transition-all duration-300 font-bold text-base border ${
                      pathname === link.href
            ...
            {/* Account Info */}
            {!loading && (
              <div className="mt-4 pt-4 border-t border-[var(--border)] w-full flex-shrink-0">
                {user ? (
                  <button
                    onClick={() => { handleLogout(); setMobileMenuOpen(false); }}
                    className="w-full text-center py-4 bg-gradient-to-r from-rose-400 to-red-500 rounded-2xl font-black text-base shadow-xl shadow-red-500/30 active:scale-95 transition-all outline-none"
                    style={{ color: 'black' }}
                  >
                    Logout
                  </button>
                  ...
                  <Link
                    href="/login"
                    onClick={() => setMobileMenuOpen(false)}
                    className="block w-full text-center py-4 bg-gradient-to-r from-[var(--primary)] to-[var(--secondary)] rounded-2xl font-black text-base shadow-xl shadow-primary/30 active:scale-95 transition-all"
                    style={{ color: 'black' }}
                  >
                    Login
                  </Link>
