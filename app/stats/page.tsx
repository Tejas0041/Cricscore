...
export default function StatsPage() {
  ...
  const battingData = (() => {
    ...
    const isAllMatches = dateFilter === 'all' && !pickedDate;
    return [...stats.battingStats]
      .filter((p: any) => {
        ...
        if (battingMetric === 'strikeRate' || battingMetric === 'average') return isAllMatches ? p.innings >= 10 : p.innings > 0;
  ...
  const bowlingData = (() => {
    ...
    return [...stats.bowlingStats]
      .filter((p: any) => {
        ...
        if (bowlingMetric === 'strikeRate') return p.wickets > 0 && (isAllMatches ? p.balls >= 90 : p.balls > 0);
        if (bowlingMetric === 'economy') return isAllMatches ? p.balls >= 90 : p.balls > 0;
        return p.balls > 0;
      })
      .sort((a: any, b: any) => {
  ...
  const winBattingData = (() => {
    ...
    return [...stats.winBattingStats]
      .filter((p: any) => {
        ...
        if (winBattingMetric === 'strikeRate' || winBattingMetric === 'average') return isAllMatches ? p.innings >= 5 : p.innings > 0;
        return p.runs > 0;
      })
      .sort((a: any, b: any) => {
  ...
  const winBowlingData = (() => {
    ...
    return [...stats.winBowlingStats]
      .filter((p: any) => {
        ...
        if (winBowlingMetric === 'strikeRate') return p.wickets > 0 && (isAllMatches ? p.balls >= 30 : p.balls > 0);
        if (winBowlingMetric === 'economy') return isAllMatches ? p.balls >= 30 : p.balls > 0;
        return p.balls > 0;
      })
      .sort((a: any, b: any) => {
      ...
      <main className="container mx-auto px-4 py-8 max-w-4xl">
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="flex flex-col gap-3 mb-8">
            {(dateFilter === 'all' && !pickedDate && category === 'batting' && (battingMetric === 'strikeRate' || battingMetric === 'average')) && (
            ...
            {(dateFilter === 'all' && !pickedDate && category === 'bowling' && (bowlingMetric === 'economy' || bowlingMetric === 'strikeRate')) && (
              <div className="text-[10px] text-amber-500/90 font-black uppercase tracking-widest sm:tracking-[0.2em] px-4 py-2.5 rounded-2xl bg-amber-500/5 border border-amber-500/20 w-full text-center shadow-inner">
                ⚠️ Minimum 15 overs (90 balls) required
              </div>
            )}
            {(dateFilter === 'all' && !pickedDate && category === 'wins' && winSubCategory === 'batting' && (winBattingMetric === 'strikeRate' || winBattingMetric === 'average')) && (
              <div className="text-[10px] text-amber-500/90 font-black uppercase tracking-widest sm:tracking-[0.2em] px-4 py-2.5 rounded-2xl bg-amber-500/5 border border-amber-500/20 w-full text-center shadow-inner">
                ⚠️ Minimum 5 innings in wins required
              </div>
            )}
            {(dateFilter === 'all' && !pickedDate && category === 'wins' && winSubCategory === 'bowling' && (winBowlingMetric === 'economy' || winBowlingMetric === 'strikeRate')) && (
              <div className="text-[10px] text-amber-500/90 font-black uppercase tracking-widest sm:tracking-[0.2em] px-4 py-2.5 rounded-2xl bg-amber-500/5 border border-amber-500/20 w-full text-center shadow-inner">
