...
export async function POST(
  ...
  try {
    ...
    // Use match's own scoring rules — avoid extra DB round-trip every ball
    const sr = match.scoringRules;
    ...
    if (isSuperOver && match.superOver?.active) {
      ...
      if (['wide', 'noball', 'deadball'].includes(eventType)) {
        ...
        if (eventType === 'run' && runs > 0) Player.findByIdAndUpdate(batsman, { $inc: { 'stats.batting.runs': runs } }).catch(() => {});
        else if (eventType === 'wicket') Player.findByIdAndUpdate(bowler, { $inc: { 'stats.bowling.wickets': 1 } }).catch(() => {});
        ...
        if (innings.overs >= so.overs || innings.wickets >= teamPlayers || targetChased) {
          ...
          if (so.currentInnings === 'first') {
            ...
            so.completed = true;
            if (so.innings.second.runs > so.innings.first.runs) so.winner = so.innings.second.battingTeam;
            else if (so.innings.first.runs > so.innings.second.runs) so.winner = so.innings.first.battingTeam;
            else so.winner = 'Tie';
      ...
      if (['wide', 'noball', 'deadball'].includes(eventType)) {
        ...
        if (innings.overs >= match.overs || innings.wickets >= teamPlayers || targetChased) {
          ...
          if (match.currentInnings === 'first') {
            ...
            if (match.innings.second.runs > match.innings.first.runs) match.winner = match.innings.second.battingTeam;
            else if (match.innings.first.runs > match.innings.second.runs) match.winner = match.innings.first.battingTeam;
    ...
    // Note: timeline.batsman/bowler NOT populated here for performance — scorer page computes stats from IDs directly
